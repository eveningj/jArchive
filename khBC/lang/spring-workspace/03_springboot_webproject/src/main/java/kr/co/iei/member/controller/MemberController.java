...
public class MemberController {
	...
	public String login(Member m, Model model, HttpSession session) {
		//로그인 -> 가입할 당시의 입력했던 아이디와 비밀번호가 일치하면 -> 회원정보를 저장해놓고 사용
		...
		if(member == null) {
			...
			//아이디 또는 패스워드를 확인하세요 라는 메세지를 보여주고 로그인 페이지로 이동
			...
			if (member.getUserLevel() == 3) {
				//2. 아이디 and 패스워드는 정상이지만 -> 듣급이 준회원 (userLevel ==3)
				//권한이 없습니다 관리자에게 문의하세요 라는 메세지를 보여주고 메인페이지
				...
				//3. 아이디 and 패스워드도 정상이고 -> 등급이 정회원 or 관리자인 경우 -> 조회결과의 embmerLevel == 1 or 2
				//로그인 -> 아이디 / 패스워드로 신원 인증
				//	-> 해당 웹사이트 이용시 내 정보를 지속적으로 사용하기 위해서
				//	-> 로그인 한 회원 정보를 페이지를 사용하는 동안 지속
				//	-> 이 때 model 로 데이터를 등록하면 해당 요청이 끝나면 그 정보가 없음.
				//	-> 해당 페이지를 이용하는 동안 데이터 저장을 유지하려면 session 객체 사용
				//	-> 세션에 저장된 데이터가 사라지는 경우
				//	-> 1) 세션 정보를 지웠을 때 (로그아웃을 직접 하는 경우)
				//	-> 2) 브라우저를 종료했을 때
				//	-> 3) 지정된 시간이 지나면 (일반적으로 30분 ~ 1시간)
				//	-> 4) 서버가 재시작되면 
	...
	public String checkId(String checkId, Model model) {
		...
		if(member == null)	{
			//입력받은 아이디로 회원을 조회한 결과가 없음 -> 가입 가능
			model.addAttribute("result", 0);
		}else {
			//입력받은 아이디로 회원을 조회한 결과가 있음 -> 가입 불가능
	...
	public String update2 (Member m, HttpSession session) {
		int result = memberService.updateMember(m);
		//session 에 저장된 회원 정보는 로그인한 시점의 DB 데이터를 조회해서 저장해둔 상태
		//회원 정보에 변화가 발생하면, 로그인한 시점의 데이터와 현재 DB가 다른 상태
