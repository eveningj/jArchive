...
//			2) 입력 숫자는 중복X
			...
			sort(num);
			/*
			 * for (int i = 0; i < num.length - 1; i++) { for (int j = 0; j < num.length - 1
			 * - i; j++) { if (num[j] > num[j + 1]) { int temp = num[j]; num[j] = num[j +
			 * 1]; num[j + 1] = temp; } } // 상위 for 문 } for (int i = 0; i < num.length; i++)
			 * { System.out.print(num[i] + "\t"); }
			 */
			System.out.println("\n컴퓨터 번호 :");
			sort(cNum);
			/*
			 * for (int i = 0; i < cNum.length - 1; i++) { for (int j = 0; j < cNum.length -
			 * 1 - i; j++) { if (cNum[j] > cNum[j + 1]) { int temp = cNum[j]; cNum[j] =
			 * cNum[j + 1]; cNum[j + 1] = temp; } } } for (int i = 0; i < cNum.length; i++)
			 * { System.out.print(cNum[i] + "\t"); } // for
			 */
			...
			case 2:
				...
				break;
			default:
				System.out.println("잘못된 입력입니다.1 , 2 중 하나의 정수를 입력해주세요.");
			}
		}
