...
//자바와 차이점은, 자바는 지정한 자료형만 여러개 저장 가능하지만, javascript 배열은 저장하는 자료형에 제한이 없음.
...
arr1[1] = 100;
console.log(arr1);
arr1[3] = 200;  //최초 선언 이후에 길이가 변경될 수 있음.
console.log(arr1);
arr1[5] = 300;
console.log(arr1);
console.log(arr1[4]);
...
//자바 스크립트에서 배열 선언하는 방법 2(배열 생성자를 사용)
...
console.log(data1);
console.log(fruits1,fruits2,testArr1,testArr2);

//join(value) : 배열 내부 데이터를 매개변수를 기준으로 구분해서 하나의 문자열로 리턴
...
//sort() : 정렬(원본배열 순서 변경) -> 매개변수를 주지 않으면 유니코드 기준 오름차순 정렬 (문자열로 변환한 후 사전순)
const testArr3 = [100,25,10,50];
console.log(testArr3);
testArr3.sort(); // 사전순 정렬
console.log(testArr3);
...
testArr4.pop();
console.log(testArr4);
//unshift(data) : 매개변수로 전달한 데이터를 배열의 가장 앞에 추가 (기존 데이터들은 인덱스 번호가 하나씩 밀림)
...
// slice : 배열의 지정된 인덱스 번호의 데이터를 복사해서 리턴 -> 원본 배열에는 변화가 없음.
const testArr5 = testArr4.slice(1,3); //배열인덱스 1번부터 3번 앞까지 복사
console.log(testArr4);
console.log(testArr5);
const testArr6 = testArr4.slice(1); //매개변수를 하나 주면 해당 인덱스부터 끝까지 복사
console.log(testArr4);
...
testArr7[3] = 20;
console.log(testArr4);  
console.log(testArr7);  //똑같은 주소.

//깊은 복사를 하고 싶으면 ?-> slice
const testArr8 = testArr4.slice(0); //0번 인덱스부터 끝까지
console.log(testArr4);  
...
//splice : 배열을 잘라내는 함수 -> 원본 배열에서 잘라낸 데이터가 사라짐
...
//splice 를 이용해 배열 중간데이터를 수정/삭제/추가하는 방법
//수정 : 배열.splice(수정하고싶은 인덱스번호, 1, 수정데이터);
//삭제 : 배열.splice(삭제하고싶은 인덱스번호, 1);
//추가 : 배열.splice(추가하고 싶은 인덱스 번호,, 0, 추가하고싶은 데이터);
