/*
자바의 메소드 선언 방법
[접근제어지시자] 리턴타입 메소드 이름 (매개변수){
    수행코드
    ...
    return 되돌려줄 데이터
}

//1. 선언적 함수 : 자바스크립트 코드가 동작할 때 가장 먼저 만들어짐
...
const data5 = data4();
console.log(data5);
//3-1 익명함수2 : function 이름을 붙이지 않음 / 변수에 저장도 안함.
    // -> 스스로 한번만 실행하고 재사용하지 않음.
    // -> 필요한 값을 초기화 할 때 사용.
(function(){
...
//함수의 매개변수
//함수의 매개변수 작성시 변수 이름은 상관없음, 순서가 중요.
//매개변수 작성 시 자료형이 의미가 없기 때문에 매개변수 이름만 작성.
function func3(num, str){
    ...
    //arguments 객체 : 함수별로 존재하는 지역변수 객체 (함수 호출시 전달하는 매개변수를 배열로 가지고 있음.)
...
func3("hello",200);
func3([1,2,3],true);
//선언한 매개변수보다 매개변수를 부족하게 전달 -> 앞에서부터 순서대로 사용하고, 전달하지않은 것은 undefined.
...
//자바스크립트 내장 함수
const url = "http://www.naver.com?query=날씨";  //최소한의 문자만 인코딩
...
console.log(isNaN(num2));   //자동 형변환으로 숫자가 변경될 수 있으면 false. -> 타입이 문자열 인데도 ! !
console.log(isNaN(num3));

console.log(isFinite(num1));
console.log(isFinite(num2));    //자동 형변환으로 숫자가 변경될 수 있으면 true. -> 타입이 문자열 인데도 ! !
